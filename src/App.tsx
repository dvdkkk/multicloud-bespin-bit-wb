...
export default function App() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Optimistic UI: 사용자가 전송 버튼을 누르면 서버 응답 대기 없이 즉시 완료 메시지 표시 (0.1초 반응)
    setIsSubmitted(true);

    // Background transmission with keepalive: true (페이지 전환이나 창 닫힘 시에도 전송 보장)
    try {
      ...
      console.error("Failed to execute background fetch:", err);
    }
  };
    ...
    <>
      ...
      <section id="overview" className="py-24 px-6">
        ...
        <div className="mt-12 flex justify-center">
          <button 
            onClick={() => scrollTo('contact-form')}
            className="bg-blue-600 text-white hover:bg-blue-700 px-10 py-4 rounded-full font-black text-lg transition-all shadow-xl hover:shadow-blue-600/40 hover:scale-105 flex items-center justify-center gap-2 group"
          >
            지금 바로 지원하기 <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
          </button>
        </div>
      </section>
      ...
      <section id="contact-form" className="py-24 px-6 bg-gradient-to-br from-blue-900 to-blue-950 relative overflow-hidden">
        ...
        <FadeIn className="max-w-7xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Right Form */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-2xl transition-all">
              {isSubmitted ? (
                <div className="py-12 px-4 text-center flex flex-col items-center justify-center min-h-[460px]">
                  ...
                  <p className="text-slate-600 text-base leading-relaxed max-w-md mb-8">
                    문의가 정상적으로 접수되었습니다.<br />
                    빠른 시일 내에 전문 상담 매니저가 남겨주신 연락처로 친절히 안내해 드리겠습니다.
                  </p>
                  <button 
                    type="button" 
                    onClick={() => setIsSubmitted(false)}
                    className="px-6 py-3 rounded-xl border border-slate-200 text-slate-700 hover:text-blue-600 hover:border-blue-200 text-sm font-bold transition-all bg-slate-50 hover:bg-white"
                  >
                    추가 문의 작성하기
                  </button>
                </div>
